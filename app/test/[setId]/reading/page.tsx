...
export default function ReadingPage({ params }: { params: Promise<{ setId: string }> }) {
  ...
  const timerColor = timeLeft < 300 ? "#dc2626" : "#1e3a8a";
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #1e3a8a, #3b5fc0)", padding: "16px 20px" }}>
      ...
      <div style={{ flex: 1, padding: "20px 16px", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {q.type === "truefalse" ? (
            ["true", "false"].map((opt) => (
              <OptionBtn key={opt} label={opt === "true" ? "True" : "False"} selected={answers[q.id] === opt} onSelect={() => setAnswers((a) => ({ ...a, [q.id]: opt }))} />
            ))
          ) : (
            q.options?.map((opt) => (
